      ...
      <div className="relative bg-[#0D0D0D] border border-[#222222] text-[#F0F0F0] rounded-2xl w-full max-w-4xl shadow-2xl overflow-hidden flex flex-col md:flex-row z-10 animate-scaleUp">
        <div className="flex-1 p-6 sm:p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-xs font-semibold text-[#888888] uppercase tracking-wider mb-1">Full Name <span className="text-rose-450">*</span></label>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-semibold text-[#888888] uppercase tracking-wider mb-1">Email Address <span className="text-rose-450">*</span></label>
              ...
              <div>
                <label className="block text-xs font-semibold text-[#888888] uppercase tracking-wider mb-1">Phone Number <span className="text-rose-450">*</span></label>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="sm:col-span-2">
                <label className="block text-xs font-semibold text-[#888888] uppercase tracking-wider mb-1">Detailed Address <span className="text-rose-450">*</span></label>
              ...
              <div>
                <label className="block text-xs font-semibold text-[#888888] uppercase tracking-wider mb-1">City <span className="text-rose-455">*</span></label>
            ...
            <div className="pt-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <label className={`p-3 border rounded-xl flex items-center justify-between cursor-pointer transition-all duration-300 ${formData.paymentMethod === 'cod' ? 'border-amber-400 bg-[#111111] shadow-sm font-medium text-white' : 'border-[#222] hover:border-[#333] text-[#888888]'}`}>
                  ...
                  <CheckCircle className={`w-4 h-4 text-amber-550 ${formData.paymentMethod === 'cod' ? 'opacity-100' : 'opacity-0'}`} />
                </label>
                ...
                <label className={`p-3 border rounded-xl flex items-center justify-between cursor-pointer transition-all duration-300 ${formData.paymentMethod === 'card' ? 'border-amber-400 bg-[#111111] shadow-sm font-medium text-white' : 'border-[#222] hover:border-[#333] text-[#888888]'}`}>
                  ...
                  <CheckCircle className={`w-4 h-4 text-amber-550 ${formData.paymentMethod === 'card' ? 'opacity-100' : 'opacity-0'}`} />
                </label>
              </div>
            </div>

            <button type="submit" disabled={isSubmitting} className="w-full bg-white hover:bg-neutral-200 text-black transition-all duration-300 py-3.5 font-sans text-sm font-bold rounded-xl shadow-md disabled:bg-[#444] disabled:text-[#888] focus:outline-none mt-4 flex items-center justify-center gap-2 cursor-pointer">{isSubmitting ? (<><div className="w-4.5 h-4.5 border-2 border-black/30 border-t-black rounded-full animate-spin"></div><span>Verifying Credentials & Stock...</span></>) : (<span>Confirm Order ({formatPrice(total, currency)})</span>)}</button>
          </form>
        </div>
        ...
        <div className="w-full md:w-80 bg-[#111111] p-6 sm:p-8 border-t md:border-t-0 md:border-l border-[#222222] flex flex-col justify-between shrink-0">
          <div>
            <h3 className="font-sans font-bold text-white text-lg mb-4 flex items-center gap-2 pb-2 border-b border-[#222222]"><ShoppingBag className="w-4.5 h-4.5 text-amber-400" />Order Summary</h3>
